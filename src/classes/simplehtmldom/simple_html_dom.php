 ...
 *
 * Changes:
 * 		manager files with php includes.
 * Version Rev. 2.0
 */
...
class simple_html_dom{
	...
	function __construct($str = null,$lowercase = true,	$forceTagsClosed = true,$target_charset = DEFAULT_TARGET_CHARSET,$stripRN = true,$defaultBRText = DEFAULT_BR_TEXT,
		$defaultSpanText = DEFAULT_SPAN_TEXT,$options = 0){
		if ($str) {
	...
	function load($str,	$lowercase = true,$stripRN = true,$defaultBRText = DEFAULT_BR_TEXT,	$defaultSpanText = DEFAULT_SPAN_TEXT,$options = 0){
		...
		$inc= new Includes();
		$this->doc=$inc->loadIncludes($this->doc);
		...
		//$aa=$this->between('include("', '");', $this->doc);
		//$this->doc = str_replace("include", 'u'. $this->doc, $this->doc);
		// strip out the \r \n's if we are told to.
		if ($stripRN) {
 ...
 /* function addIncludes($doc){
    if (str_contains($doc, "<?php include"): bool)
